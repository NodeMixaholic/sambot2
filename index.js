...
client.on('message', async (message) => {
	...
	try {
		...
		if (obj.attributeScores.TOXICITY.summaryScore.value > .8 || text.toLowerCase().includes("cum") || (text.toLowerCase().includes("screen") && text.toLowerCase().includes("nft")) || text.toLowerCase().includes("pssy") || text.toLowerCase().includes("btch") || text.toLowerCase().includes("flck") || text.toLowerCase().includes("fIck") || text.toLowerCase().includes("f^ck") || text.toLowerCase().includes("fxck") || text.toLowerCase().includes("fck") || text.toLowerCase().includes("shifty shafts") || text.toLowerCase().includes("sex") || text.toLowerCase().includes("shxt") || text.toLowerCase().includes("sxht")) {
			message.delete();
